...
主窗口的“解说剪辑”页面用于管理 Codex 编排后的结构化解说项目。它支持项目
JSON、`narration/original` 交替时间轴、固定男女声纹试听、源区间校验、源节目响度
测量、单段局部渲染、完整预览和最终成片。渲染会分别生成无声视频、解说、原声、
完整混音四套母版，并在全片段画面感知去重通过后封装最终 MP4。

命令行入口为 `py -3.12 recap_cli.py --help`，项目示例位于
`recap/examples/project.example.json`，正式声纹库位于 `recap/voices/library.json`。
完整工作规范和实现边界分别见 `CODEX_RECAP_INIT.md` 与
`RECAP_ENGINE_IMPLEMENTATION.md`。程序不内置剧情理解 Agent；外部 Codex 对话负责
理解剧情和修改时间轴，程序负责确定性校验、版本、缓存与渲染。
