...
const projectMaterialPattern=/\.(png|jpe?g|webp|bmp|txt|md)$/i;
const projectCoverPattern=/\.(png|jpe?g|webp|bmp)$/i;
const projectInformationPattern=/\.(txt|md)$/i;
function selectProjectCover(files){return files.filter(x=>projectCoverPattern.test(x.name)).sort((a,b)=>Number(/^cover\./i.test(b.name))-Number(/^cover\./i.test(a.name))||a.name.localeCompare(b.name))[0]}
function selectProjectInformation(files){return files.filter(x=>projectInformationPattern.test(x.name)).sort((a,b)=>Number(/\.md$/i.test(b.name))-Number(/\.md$/i.test(a.name))||a.name.localeCompare(b.name))[0]}
...
function selectedProjectFiles(){if(sourceDirectoryHandle)return directoryProjectFiles;const files=[...$('folderFiles').files].filter(file=>{const parts=relativeParts(file);return projectMaterialPattern.test(file.name)&&parts.length===2&&!/^(?:TikTok发布信息|bio)\.txt$/i.test(file.name)});return[selectProjectCover(files),selectProjectInformation(files)].filter(Boolean)}
async function readProjectMaterials(handle){const files=[];for await(const entry of handle.values())if(entry.kind==='file'&&projectMaterialPattern.test(entry.name)&&!/^(?:TikTok发布信息|bio)\.txt$/i.test(entry.name))files.push(await entry.getFile());return[selectProjectCover(files),selectProjectInformation(files)].filter(Boolean)}
