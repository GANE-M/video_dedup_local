...
function renderVideoSelection(){const files=selectedVideoFiles(),ignored=sourceDirectoryHandle?0:[...$('folderFiles').files].filter(file=>videoSuffixPattern.test(file.name)&&!isDirectFolderVideo(file)).length;$('fileList').innerHTML=files.map(file=>`<li>${escapeHtml(file.webkitRelativePath||file.name)} · ${(file.size/1048576).toFixed(1)} MB</li>`).join('')+(ignored?`<li class="muted">已忽略子文件夹中的 ${ignored} 个视频</li>`:'');if(files.length){$('series').value=inferredSeriesName(files);$('recapProjectName').value=$('series').value;loadPreview(files[0])}persist()}
function clearDirectorySelection(){sourceDirectoryHandle=null;directoryVideoFiles=[];directorySubtitleFiles=[];selectedSourceFolderName=''}
