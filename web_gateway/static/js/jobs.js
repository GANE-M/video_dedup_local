$('start').onclick=async()=>{try{
  const entries=selectedEntries(),videos=entries.filter(x=>x.role==='video'),series=$('series').value.trim();
  const subtitle=$('enableSubtitles').checked,publishing=$('enablePublishing').checked,recap=$('enableRecap').checked,dedup=$('enableDedup').checked;
  if(!subtitle&&!publishing&&!recap&&!dedup)throw new Error('请至少勾选字幕、发布物料、解说或去重中的一个阶段');
  if(recap&&dedup)throw new Error('解说成片生成后才能执行二次去重；当前请先运行“字幕 + 解说”，成片确认后再单独运行去重');
  if(!$('apiKey').value.trim()||!series||!videos.length)throw new Error('请填写密钥、剧名并选择视频');
  if(dedup&&!outputDirectoryHandle)throw new Error('启用去重时请选择成片保存文件夹');
  if($('translationBackend').value==='api'&&subtitle&&$('llmApiKey').value.trim().length<8)throw new Error('API模式需要填写LLM API Key');
  $('start').disabled=true;$('message').textContent='创建任务…';
  const settings=settingsPayload();
  const body={series_name:series,project_name:selectedSourceFolderName||inferredSeriesName(videos.map(x=>x.file))||series,files:entries.map(x=>({name:x.file.name,size:x.file.size,role:x.role})),settings,llm_api_key:$('translationBackend').value==='api'&&subtitle?$('llmApiKey').value.trim():null};
  current=await api('/api/v1/jobs',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});showJob(current);
  if(publishing||(subtitle&&$('translationBackend').value==='agent'))showAgent(current.agent_bootstrap);
  $('resultCard').classList.remove('hidden');$('cancel').disabled=false;lastEvent=0;$('events').textContent='等待日志…';
  await uploadPool(entries,current.uploads,current.chunk_size);await api(`/api/v1/jobs/${current.id}/start`,{method:'POST'});
  $('message').textContent=publishing?'上传完成，发布物料 Agent 将先生成封面方案和 TikTok 文案':(recap?(subtitle?'上传完成，后端将依次执行字幕与解说':'上传完成，后端正在建立解说项目'):'上传完成，已加入处理队列');
  await refreshJobs();poll()
...
function processedOutputArtifacts(items=latestArtifacts){return items.filter(item=>item.path.startsWith('videos/')&&!item.path.startsWith('videos/recap/'))}
...
async function chooseSourceFolder(){if(!window.showDirectoryPicker)throw new Error('当前浏览器不支持工程文件夹授权，请使用最新版 Chrome 或 Edge');const handle=await window.showDirectoryPicker({mode:'readwrite'}),files=[];for await(const entry of handle.values())if(entry.kind==='file'&&videoSuffixPattern.test(entry.name))files.push(await entry.getFile());files.sort((a,b)=>a.name.localeCompare(b.name,undefined,{numeric:true,sensitivity:'base'}));sourceDirectoryHandle=handle;directoryVideoFiles=files;directorySubtitleFiles=await readSubtitleFinals(handle);directoryProjectFiles=await readProjectMaterials(handle);selectedSourceFolderName=handle.name;outputDirectoryHandle=await handle.getDirectoryHandle('processed',{create:true});$('outputFolderStatus').textContent=`默认保存到：${handle.name}/processed（可手动更换）`;$('files').value='';$('folderFiles').value='';renderVideoSelection();const activeCount=activeSubtitleFiles(files).length;$('message').textContent=`工程已载入：${files.length} 个一级视频、${directoryProjectFiles.length} 个发布素材；将上传 ${activeCount} 个活动字幕（每集最多原文+目标译文），已忽略子目录和历史字幕`;if(!files.length)throw new Error('所选文件夹第一层没有视频')}
async function chooseOutputFolder(){if(!window.showDirectoryPicker)throw new Error('当前浏览器不支持文件夹直写，请使用最新版 Chrome 或 Edge');outputDirectoryHandle=await window.showDirectoryPicker({mode:'readwrite'});$('outputFolderStatus').textContent=`保存到：${outputDirectoryHandle.name}`;$('saveArtifacts').disabled=!processedOutputArtifacts().length&&!recapVideoArtifacts().length}
...
async function saveVideosToFolder(items=latestArtifacts){const outputs=processedOutputArtifacts(items),recaps=recapVideoArtifacts(items),projectArtifacts=items.filter(x=>/^(subtitles|logs|agent|config)\//.test(x.path));if(!outputs.length&&!recaps.length&&!projectArtifacts.length)throw new Error('当前任务没有可保存的成片、封面、发布文案、字幕或记录');if(outputs.length)await requireWritePermission(outputDirectoryHandle,'成片保存文件夹');if(sourceDirectoryHandle&&(recaps.length||projectArtifacts.length))await requireWritePermission(sourceDirectoryHandle,'工程文件夹');const recordName=current?.result?.record_name||current?.id||'unknown-job';let completed=0,projectFiles=0;for(const artifact of outputs){$('message').textContent=`保存 processed 成果 ${completed+1}/${outputs.length}：${artifact.path.split('/').at(-1)}`;await writeArtifact(outputDirectoryHandle,artifact.path.slice('videos/'.length),await fetchArtifactBlob(artifact.path));completed++}if(sourceDirectoryHandle){for(const artifact of recaps){const target=`解说/${artifact.path.slice('videos/recap/'.length)}`;$('message').textContent=`保存解说成片：${target}`;await writeArtifact(sourceDirectoryHandle,target,await fetchArtifactBlob(artifact.path));completed++}for(const artifact of projectArtifacts){const target=artifact.path.startsWith('subtitles/')?`字幕终稿/${artifact.path.slice('subtitles/'.length)}`:`任务记录/${recordName}/${artifact.path}`;$('message').textContent=`同步工程记录：${target}`;await writeArtifact(sourceDirectoryHandle,target,await fetchArtifactBlob(artifact.path));projectFiles++}}else if(recaps.length){for(const artifact of recaps){await writeArtifact(outputDirectoryHandle,`解说/${artifact.path.slice('videos/recap/'.length)}`,await fetchArtifactBlob(artifact.path));completed++}}$('message').textContent=(completed?`已保存 ${completed} 个成片/封面/文案成果`:'未生成输出成果')+(sourceDirectoryHandle?`，并同步 ${projectFiles} 个字幕/记录文件到 ${sourceDirectoryHandle.name}`:'')}
...
async function poll(){
  ...
  try{
    ...
    if(job.status==='waiting_publishing_agent')$('message').textContent='正在等待 Agent 生成封面方案、标题、Bio 和 hashtag';
